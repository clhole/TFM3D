...
% PRESET CONSTANTS
maxIterations = 20; % maximum number of iterations; 20 after debugging
dm = 8; % desired output mesh spacing was 8 at the start
convergenceCrit = [0.25, 0.5, 0.0625]; % convergence criteria
ccThreshold = 1e-4; % bad cross-correlation threshold

[I0, sSize, sSpacing, padSize, DVCPadSize, u, vxsize, range] = parseInputs(varargin{:});
...
[nodes, u] = parseOutputs(u,finalSize,padSize,mFinal,vxsize,range);
delete(wb);
...
I0{1} = single(varargin{1});
I0{2} = single(varargin{2});
...
varargout{end+1} = u0;
varargout{end+1} = varargin{4};
varargout{end+1} = varargin{5};
end


function [nodes, u] = parseOutputs(u,finalSize,padSize,m,vxsize,range)
% parses outputs and unpads the displacment field and cc.
filterSpacing = finalSize/2;

for i = 1:3
    m{i} = m{i}-padSize(1,i)-filterSpacing(i);
end

% Something with the ordering of x,y,z seems very strange, this might
%be wrong here !!!!!

nodes(:,1) = (m{1}(:)-1)*vxsize(1)+range(1,1);
nodes(:,2) = (m{2}(:)-1)*vxsize(2)+range(2,1);
nodes(:,3) = (m{3}(:)-1)*vxsize(3)+range(3,1);

v=u;
clear u;

% the negative comes from the ordering of img0 and img1
u(:,1) = -v{2}(:)*vxsize(1);
u(:,2) = -v{1}(:)*vxsize(2);
u(:,3) = -v{3}(:)*vxsize(3);
