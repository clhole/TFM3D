...
/CWD,'H:\TFM 3D\Traction\FEA\work'
...
~SATIN,'cellmodel','sat','H:\TFM 3D\Traction\FEA\work',SOLIDS,0
						!Load cell model
BLOCK,rng(1,1),rng(1,2),rng(2,1),0,rng(3,1),rng(3,2)
						!Create substrate bulk
VSBV,2,1!,SEPO,,,DELETE					!Subtract cell from the substrate bulk
...
*DO,I,1,nnodes
   ...
   !nmin = NDNEXT(nmin)			!Next higher node number in the selection
*ENDDO

*DIM,tt,ARRAY,nnodes,5				!Define array for stresses
*DO,I,1,nnodes
   tt(I,1)=I 					!Node number
   tt(I,2)=nodes(I,1)				!Nodal x coordinates
   tt(I,3)=nodes(I,2)				!Nodal y coordinates
   tt(I,4)=nodes(I,3)				!Nodal z coordinates
   tt(I,5)=nmin				!Nodal z coordinates
	nmin = NDNEXT(nmin)			!Next higher node number in the selection

*ENDDO

*CFOPEN,test,txt 				!Create file
*CFWRITE,Node,N,x,y,z
*DO,I,1,nnodes					!Write data to file
   *CFWRITE,Node,tt(I,1),tt(I,2),tt(I,3),tt(I,4),tt(I,5)
*ENDDO
*CFCLOS
